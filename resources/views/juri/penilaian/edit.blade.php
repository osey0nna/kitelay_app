<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between relative z-10">
            <div class="flex flex-col gap-2">
                ...
                <p class="text-sm font-medium text-slate-400">{{ $perlombaan->nama_lomba }} <span class="mx-2 text-neutral-600">|</span> <span class="text-white">{{ $pendaftaran->user->name }}</span></p>
            </div>
    ...
    <div class="py-10 bg-black min-h-screen relative overflow-hidden" x-data="{ mounted: false }" x-init="setTimeout(() => mounted = true, 50)">
        ...
        <div class="mx-auto flex max-w-5xl flex-col gap-8 px-4 sm:px-6 lg:px-8 relative z-10 transition-all duration-700 ease-out"
