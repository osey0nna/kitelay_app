<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 relative z-10">
            ...
            <p class="text-xs md:text-sm font-medium text-slate-400">Kelola akun admin, juri, dan pendaftar dengan struktur yang lebih rapi.</p>
        </div>
    </x-slot>
    ...
    <div class="py-6 md:py-10 bg-black min-h-screen relative overflow-hidden" x-data="{ mounted: false }" x-init="setTimeout(() => mounted = true, 50)">
        ...
        <div class="mx-auto flex max-w-7xl flex-col gap-8 px-4 md:px-6 lg:px-8 relative z-10 transition-all duration-700 ease-out"
            ...
            <div class="flex justify-end">
                <a href="{{ route('admin.users.create') }}" class="inline-flex items-center justify-center gap-2 rounded-sm bg-gradient-to-r from-red-600 to-red-800 px-4 md:px-6 py-2 md:py-3 text-[11px] font-black uppercase tracking-widest text-white shadow-[0_0_15px_rgba(220,38,38,0.3)] hover:scale-105 transition-all skew-x-[-10deg]">
                                ...
                                <tr class="tilt-row transition-colors hover:bg-[#0a0a0c]">
                                    ...
                                    <td class="px-6 py-6">
                                        <div class="flex justify-end gap-3">
                                            <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center justify-center rounded-sm border border-neutral-700 bg-black px-4 md:px-5 py-2 md:py-2.5 text-[11px] font-black uppercase tracking-widest text-slate-300 transition-all hover:border-amber-400 hover:text-amber-400 skew-x-[-10deg]">
                                            ...
                                            <form method="POST" action="{{ route('admin.users.destroy', $user) }}" onsubmit="return confirm('Hapus akun ini?')">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="inline-flex items-center justify-center rounded-sm border border-neutral-700 bg-black px-4 md:px-5 py-2 md:py-2.5 text-[11px] font-black uppercase tracking-widest text-red-500 transition-all hover:border-red-500 skew-x-[-10deg]">
